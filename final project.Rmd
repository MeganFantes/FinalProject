...
Tweets in Boston are most highly concetrated in the areas of the city known as Government Center and the Financial District. This is not surprising, as these two areas genrally define the "downtown" area of Boston, i.e. where there is the highest contration of people.
...
shinyApp(ui = fluidPage(sidebarLayout(
         ...
         onStart = getWords <- memoise(function(category) {
             ...
             text <- switch(category,
                            "b.tweets" = bostonText,
                            "b.hashtags" = boston.htags,
                            "s.tweets" = sydneyText,
                            "s.hashtags" = sydney.htags)
...
We will run 4 t-tests, one test to compare each of the four qualities we have measured on the tweets in each city. In a t-test, we are comparing the means and variances of two samples to see if they have equal means. For example, in the t-test for number of words per tweet, we will compare the number of words in every tweet in boston to the number of words in every tweet in Sydney to see if the *average* number of words per tweet is *different* between the cities. If the means are different, then the p-value for the test will be < 0.05. Below is a table summarizing the results of the t-tests, listing the quality that was being tested, the p-value for the test, and the city that has the **larger** mean in that category.
